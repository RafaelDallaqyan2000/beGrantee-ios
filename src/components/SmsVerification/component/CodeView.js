...
const CodeView = props => {
  ...
  const cPaddingVertical = containerPaddingVertical
    ? {paddingVertical: containerPaddingVertical}
    : {};
  const cPaddingTop = containerPaddingTop
    ? {paddingTop: containerPaddingTop}
    : {};
  const cPaddingBottom = containerPaddingBottom
    ? {paddingBottom: containerPaddingBottom}
    : {};
  const cPaddingHorizontal = containerPaddingHorizontal
    ? {paddingHorizontal: containerPaddingHorizontal}
    : {};
  const cPaddingLeft = containerPaddingLeft
    ? {paddingLeft: containerPaddingLeft}
    : {};
  const cPaddingRight = containerPaddingRight
    ? {paddingRight: containerPaddingRight}
    : {};
  const cBackgroundColor = containerBackgroundColor
    ? {backgroundColor: containerBackgroundColor}
    : {};
  const cMarginLeft = {marginLeft: containerMarginLeft};
  ...
  return (
    <View
      ...
      {codeArray.map((code, index) => {
        ...
        const viewHight = codeViewHeight
          ? {height: codeViewHeight}
          : {height: codeViewWidth};
        const viewBackgroundColor = code
          ? codeViewBackgroundColor
            ? {backgroundColor: codeViewBackgroundColor}
            : {}
          : {};
        const viewBorderWidth = codeViewBorderWidth
          ? {borderWidth: codeViewBorderWidth}
          : {};
        const viewBorderRadius = codeViewBorderRadius
          ? {borderRadius: codeViewBorderRadius}
          : {};
        const viewBorderColor = code
          ? codeViewBorderColor
            ? {borderColor: codeViewBorderColor}
            : {}
          : {borderColor: errorBorderColor ? errorBorderColor : '#EAECF0'};
        const focusedViewBorderColor = focusedCodeViewBorderColor
          ? {borderColor: focusedCodeViewBorderColor}
          : {};
        const vbColor =
          foucsedIndex === index && focused
            ? focusedViewBorderColor
            : viewBorderColor;

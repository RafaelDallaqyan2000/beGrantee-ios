...
export const defaultWindowSizes: WindowSizes = {
  base: 0,
  sm: 600,
  md: 905,
  lg: 1240,
...
export const WindowSizeClassProvider: React.FC<
  WindowSizeClassProviderProps
> = ({windowSizes = defaultWindowSizes, children}) => {
  const {width} = useWindowDimensions();

  const windowSizeClass: WindowSizeClass = useMemo(() => {
    if (width >= windowSizes.base && width < windowSizes.sm) {
      return 'base';
    }
    if (width >= windowSizes.sm && width < windowSizes.md) {
      return 'sm';
    }
    if (width >= windowSizes.md && width < windowSizes.lg) {
      return 'md';
    }
    if (width >= windowSizes.lg && width < windowSizes.xl) {
      return 'lg';
    }
    return 'xl';
  }, [width, windowSizes]);
